...
{\centering
	\chapter{Fundación}\label{parte1}
}
...
\begin{minipage}[t]{0.48\textwidth}
	\raggedright
	\emph{
		$\!\!$Un árbol que abrazan dos brazos\\
		nace de una minúscula semilla;\\[0.5em]
		una torre de nueve pisos\\
		se eleva desde un montón de tierra;\\[0.5em]
		un viaje de mil li\\
...
\vspace{1em}
\hfill { 道德經 — \textsc{Dàodé jīng, Capítulo 64}}

\section{围棋—Wéi Qí—, historia y reglas}
\setcounter{page}{1}

\subsection{Historia del Go}

El término 围棋 (Wéi Qí), pronunciado aproximadamente como «ueei chii», proviene de los caracteres 围 (Wéi), que significa «rodear», y 棋 (Qí), que refiere a un «juego de tablero». En Occidente se le conoce comúnmente como Go, término que deriva del japonés 囲碁 (Igo). Aunque se desconoce su cronología exacta, antiguos registros literarios, pictóricos y arqueológicos demuestran que el juego constituía una parte integral de la cultura china.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.51\linewidth]{figures/Go_1200x1200}
	\caption{Una partida de Go sobre un tablero tradicional.}
...
La primera mención fehaciente del juego data del siglo III a. e. c., cuando Mèngzǐ (孟軻) describe a Yì Qiū (弈秋), maestro del juego durante la era de los Reinos Combatientes reconocido como uno de los mayores exponentes de su época. Por otra parte, destaca la partida preservada en el Wàngyōu Qīnglè Jí (忘憂清樂集), una compilación de mediados del siglo XII. En dicha obra se registra una de las partidas más antiguas de las que se tiene constancia, correspondiente al período Han (siglos II a. e. c. - II e. c.) y disputada por el general Sūn Cè (孙策) y su capitán Lǚ Fàn Figura~\ref{fig:Fig 1} \cite{Hist}. En conclusión, si bien es poco probable que el Go posea 4000 años de antigüedad —como sugieren ciertas tradiciones—, su existencia está documentada desde hace al menos 2500 años. Su mención recurrente en textos y relatos de la época permite inferir que, para entonces, el juego gozaba de un conocimiento popular lo suficientemente extendido como para que las analogías construidas sobre él resultaran comprensibles.
\begin{figure}
	\centering
	...
	\end{tikzpicture}
	\caption{Partida entre \textit{Sūn Cè} y \textit{Lǚ Fàn}. Las piedras sin número se deben a que, en aquellos tiempos, se comenzaba con una posición inicial dada y además, el blanco era el primero en jugar.}
	\label{fig:Fig 1} % Assign a unique label
\end{figure}

\subsection{Historia del Go en la Argentina}

El ingeniero Hilario Fernández Long fue el precursor del Go en Argentina. Tras aprender los fundamentos de manos de un amigo japonés, su entusiasmo lo llevó a dictar los primeros cursos sobre la disciplina en el Centro Argentino de Ingenieros. Gracias a su labor, el 11 de septiembre de 1971 se fundó la Asociación Argentina de Go. Posteriormente, en 1974, Fernández Long publicó junto a Adalberto Moderc el \textit{Manual de Go}, al que siguió en 1983 la obra \textit{Introducción al Go} \cite{Long83}.  

Resulta imperativo destacar la figura de Fernando Aguilar \cite{Long_Hubert, Aguilar2, Aguilar1}, sobrino segundo de Fernández Long y, probablemente, el máximo exponente del Go en nuestro país. Aunque sus primeros pasos en el mundo de los juegos de estrategia los dio en el ajedrez, bajo la tutela de su padre, Pedro Antonio Aguilar, Fernando descubrió el Go a través de la prensa gráfica:
\begin{quote}
...
\end{quote}
Motivado por la curiosidad, consultó con su padre y este contestó:
\begin{quote}
	«Vayamos a visitar a tu tío Hilario para que nos enseñe a jugar al Go».
\end{quote}
Aguilar, a sus 12 años~\cite{Aguilar3}, recuerda así sus inicios:
\begin{quote}
...
\end{quote}
Una vez aprendidas las reglas, era hora de competir.
\begin{quote}
...
\end{quote}
Luego de la partida entre \textit{Enrique Lindenbaum} e \textit{Iwamoto}, era el turno de los aficionados:
\begin{quote}
	«Luego vino la sesión de simultáneas, donde tuve la primera experiencia de enfrentar a una jugadora profesional. Yo tenía nivel de principiante (o poco más) y debía poner 9 piedras, pero \textit{Sachiko Kondo} pidió jugar con 7 piedras [...] Cuando quedó claro que la victoria de Blanco era aplastante, la profesional pidió suspender el juego».\footnote{Al ser Aguilar el jugador «débil», este juega primero (con negras), quedando a \textit{Kondo} el jugar con blancas.}
\end{quote}
A pesar de la derrota, la ambición de \textit{Fernando} solo iba en aumento:
\begin{quote}
...
\end{quote}
Si bien no tuvo una gran participación, sí lo hizo al año siguiente.
\begin{quote}
...
\end{quote}
El hito más significativo de su carrera ocurrió en el año $2002$. Tras obtener el tercer puesto en la \textit{Copa Iberoamericana}, clasificó a la primera \textit{Toyota and Denso Cup - World Go Oza}. 

En la primera ronda, el 19 de marzo, se enfrentó a \textit{Hasegawa Sunao} ($9p$), Figura~\ref{fig:aguilar_hasegawa_1}. El inicio fue protocolar:

\begin{quote}
	«Tomé una cantidad abundante de piedras y mi adversario adivinó correctamente que se trataba de un número impar, así que le correspondió jugar con negras».\footnote{Para determinar quién juega primero, la tradición japonesa utiliza un método llamado \textit{Nigiri}(握り). El jugador de mayor rango o edad toma un puñado de piedras blancas y las oculta en su mano. Su oponente coloca en el tablero una piedra negra para adivinar que la cantidad oculta es impar, o dos piedras negras si estima que es par. Seguidamente, las piedras blancas se cuentan de a pares; si el oponente adivina correctamente, juega con las piedras negras y realiza el primer movimiento. Si se equivoca, juega con las blancas.}
\end{quote}
Contra todo pronóstico, la partida resultó a favor de \textit{Aguilar}:
\begin{quote}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Hasegawa Sunao (N) vs. Fernando Aguilar (B) - Toyota-Denso 2002, Tablero final. En azul los territorios del negro, en verde los del blanco. Las piedras con puntos con color se consideran capturadas.}
	\label{fig:aguilar_hasegawa_1}
\end{figure}
Para la segunda ronda, disputada el $2$ de septiembre, Aguilar se preparó exhaustivamente para enfrentar a \textit{Yo Kagen}, Figura~\ref{fig:aguilar_kagen_1}. Su análisis previo revela la profundidad con la que percibía el juego:
\begin{quote}
...
\end{quote}
Antes de partir a \textit{Tokio, Japón}, lugar donde se realizaba el torneo, \textit{Fernando} relata:
\begin{quote}
...
\end{quote} 
Una vez allí, logra vencer a \textit{Yo Kagen}. 
\begin{quote}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.45]
		...
		\filldraw[OliveGreen] (10,11) circle (6pt);
	\end{tikzpicture}
	\caption{Fernando Aguilar (N) vs. Yo Kagen (B) - Toyota-Denso 2002, Tablero final. Aunque el lector aún no pueda \textit{leer} el tablero, al finalizar la Parte~\ref{parte2} contará con las herramientas necesarias para hacerlo.}
	\label{fig:aguilar_kagen_1}
\end{figure}
Aunque en la tercera ronda cayó ante el legendario \textit{Lee Chang-ho}, Figura~\ref{fig:aguilar_chang}. La gesta de Aguilar permanece como un pilar del Go occidental. Cabe mencionar también su notable tercer puesto en el \textit{World Amateur Go Championship} de 2008. 
Este pequeño recorte no hace justicia a la epopeya de Fernando; en caso de estar interesado en profundizar en el análisis de las partidas, se recomienda consultar \cite{Aguilar2, Aguilar1}. 
\begin{figure}
	\centering
	\begin{tikzpicture}[scale=0.45]
		\tikzstyle{every node}=[font=\small]
		...
		\filldraw[draw=black,fill=white] (13,10) circle (12pt) ; % W[ni]
	\end{tikzpicture}
	\caption{Lee Chang Ho (N) vs. Fernando Aguilar (B)- Toyota-Denso 2002, Tablero final. Aguilar abandona la partida luego del \textcolor{Red}{turno} $125$.}
	\label{fig:aguilar_chang}
\end{figure}
\subsection{Historia de los modelos del Go}\label{HistGo}
La amplia literatura sobre el Go suele centrarse en su complejidad o en la búsqueda de estrategias óptimas, abordándolas generalmente desde la \textit{teoría de juegos combinatorios}. Este texto se inscribe entre los escasos trabajos enfocados en \textit{modelar} el Go. Esta sección está dedicada a trazar el recorrido histórico de dichos modelos.

El primer escrito sobre un posible modelo matemático del Go fue elaborado por \textit{Edward O. Thorp} y \textit{William E. Walden} en 1964 \cite{TandW64}. En este artículo, el tablero se generaliza de un tamaño fijo a uno de $n \times m$. Se establecen las definiciones de \textit{adyacencia}, los tres estados de cualquier punto y una formalización básica de las reglas. Además, definen el Computer Go — un juego finito y libre de ciclos infinitos —$\,$ para estudiar estrategias óptimas en tableros pequeños. También introducen el \textit{Go $t$-truncado}, una variante que termina forzosamente en empate en el turno $t$ (si no lo ha hecho antes).

En 1970, \textit{Albert L. Zobrist} publicó un método para codificar un tablero en un único número de gran tamaño con el objetivo de verificar sus estados \cite{zobrist1970hashing}. Aunque su trabajo no trata sobre el Go propiamente dicho, el método que describe será utilizado en la Sección~\ref{ZobristHash} para dar cuenta de cuándo se rompe una de las reglas del juego: el \textit{SuperKo}.

En 1972, Thorp y Walden publicaron un trabajo combinatorio que explora la naturaleza y reglas del Go para generar un modelo computacional. En 1976, David B. Benson \cite{Benson} se basó en Thorp y Walden para determinar formalmente cuándo un grupo está «vivo». Aunque a priori este trabajo podría parecer ajeno a la categoría de modelos generales, es pionero en intentar modelar la dinámica interna del Go. Su propuesta será revisada bajo nuestra propia notación en la Sección~\ref{sec-vida}.

Finalmente, cabe destacar el trabajo de \textit{Jin Bai Kim}, concebido en 1976 y publicado en dos partes en 1978 \cite{KJB1, KJB2}. Esta obra continúa la tradición de usar grafos para modelar el Go, aunque Kim omite uno de los estados posibles del tablero y propone una manera alternativa de definir las \textit{adyacencias}. En el segundo de estos artículos, vemos la primera formulación del Go en tres dimensiones (que estudiaremos en la Parte~\ref{parte3}).
\subsection{Formato y reglas}
...
\begin{wrapfigure}[11]{r}{0.5\textwidth}
	...
	\begin{tikzpicture}[scale=0.485] \tikzstyle{every node}=[font=\small]
		...
		\end{scope}
	\end{tikzpicture}
	\caption{Tablero vacío y un tablero con las libertades de una piedra y de grupos.}
	\label{fig:Fig 3}
\end{wrapfigure} El Go se juega sobre un tablero cuadrado compuesto por una cuadrícula de líneas que se intersecan. Consta de dos jugadores: uno controla las piezas negras y el otro, las blancas. Los tamaños de tablero más habituales son $9 \times 9$, $13 \times 13$ y $19 \times 19$. Léase «nueve por nueve», «trece por trece», etc. Para explicar las reglas utilizaremos un tablero de $7 \times 7$, un tamaño más que suficiente para este propósito. Como ilustra la Figura~\ref{fig:aguilar_chang}, las coordenadas se leen desde la esquina inferior izquierda, punto (1, 1), donde las coordenadas crecen hacia la derecha y hacia arriba. La partida comienza con el tablero vacío, Figura~\ref{fig:Fig 3}, a menos que se juegue con ventaja. Los pequeños puntos negros que se observan en algunas intersecciones sirven únicamente como referencia visual y para ubicar de forma estándar las piedras de ventaja. Durante su turno, un jugador puede realizar una de dos acciones: colocar una pieza (denominada \textit{piedra}) en una intersección libre, o bien pasar el turno. El jugador que controla las piedras negras juega primero y puede ubicar su piedra en cualquier intersección que desee. Los puntos en un tablero tienen asociadas sus \textit{adyacencias}. Estas corresponden a otros puntos que se encuentran a una distancia de una unidad, ya sea en dirección horizontal o vertical. Al posicionar una piedra en el tablero, esta adquiere \textit{libertades}, las cuales se definen como las intersecciones vacías que son \textit{adyacentes} a la piedra o a un \textit{grupo}. Dos o más piedras adyacentes del mismo color conforman un \textit{grupo}; los grupos comparten las \textit{libertades} de cada una de las piedras individuales que los componen, véase Figura~\ref{fig:Fig 3}.
...
\begin{wrapfigure}[19]{r}{0.55\textwidth}
	...
	\begin{tikzpicture}[scale=0.485] \tikzstyle{every node}=[font=\small]
		...
		\end{scope}
	\end{tikzpicture}
	\caption{Captura de una piedra blanca y de un grupo negro.}
	\label{fig:Fig 4}
\end{wrapfigure} Puede observarse que, en el segundo tablero la piedra negra posee 4 libertades y, el grupo de piedras blancas cuenta con 6 libertades, mientras que el de las negras posee 5. Durante el juego, una piedra o un grupo puede ser capturado; esto se logra al \textit{rodearlo completamente}, quitando todas las \textit{libertades} que posee. En la Figura~\ref{fig:Fig 4} se ilustra la captura de una piedra y un grupo. Al producirse una captura, las piedras involucradas se retiran del tablero. Para que una piedra pueda colocarse en el tablero, debe contar con libertades en la posición donde se desea jugar. La \textit{regla del suicidio} prohíbe colocar piedras en espacios sin libertades. Jugar en dichas intersecciones implicaría la captura inmediata de la piedra, equivaliendo a jugar contra uno mismo. Del mismo modo, si a un grupo de piedras propias le queda una única libertad, ocupar ese último espacio vital también constituye un suicidio y es, por tanto, un movimiento ilegal, véase Figura~\ref{fig:suicidio}.
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[scale=0.485] \tikzstyle{every node}=[font=\small]
		% Draw the background
		...
		\end{scope}
	\end{tikzpicture}
	\caption{En azul las jugadas ilegales para negro pues son suicidio.}
	\label{fig:suicidio}
\end{figure} 

Existe, sin embargo, una excepción a esta regla: es legal jugar en una posición sin libertades si dicho movimiento realiza una captura. Al retirar las piedras capturadas, la piedra recién colocada obtiene libertades de manera inmediata, véase Figura~\ref{fig:Fig 6}.\label{primerreglako}
...
Ademas, en la figura, si ambos jugadores pudieran jugar en la intersección, el ciclo de capturas mutuas sería interminable. Para evitar este bucle existe la \textit{regla del ko}. Si el negro jugara en la coordenada $(4,4)$ (marcada con un punto azul en la figura), el tablero volvería exactamente a la posición del turno anterior. La \textit{regla del ko} prohíbe esto.\footnote{Veremos que la \textit{regla del ko} resulta insuficiente en ciertos escenarios, ya que pueden producirse ciclos más extensos que, sin violar esta regla, repiten una posición, véase la Sección~\ref{sec-superko}.}
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[scale=0.485] \tikzstyle{every node}=[font=\small]
		% Draw the background
		...
		\end{scope}
	\end{tikzpicture}
	\caption{Posición ilegal ya que se vuelve a una posición anterior. Tras la jugada del negro en $(4, 6)$, podría recapturar en $(4, 4)$ en un turno posterior. El blanco evita esta posibilidad jugando en $(4, 4)$.}
	\label{fig:Fig 6}
\end{figure} 

Para recapturar, el negro debe jugar antes en otra parte del tablero (amenaza de ko). Esto crea una configuración distinta; si el oponente responde, el negro podrá jugar en (4, 4) y capturar, ya que el tablero no será idéntico al anterior, véase el ultimo tablero de la  Figura~\ref{fig:Fig 6}.
%		\begin{figure}[H]
	%			\centering
	%			\begin{tikzpicture}[scale=0.485] \tikzstyle{every node}=[font=\small]
		%				% Draw the background
...
\begin{wrapfigure}[11]{l}{0.3\textwidth}
	...
	\end{tikzpicture}
	\caption{\textcolor{OliveGreen}{Territorio} negro, \textcolor{Blue}{Territorio} blanco y \textcolor{Orange}{libre}.}
	\label{fig:Fig 9}
\end{wrapfigure}Se considera que una intersección vacía (o un grupo de ellas) constituye el \textit{territorio} de un color si está completamente rodeada por piedras de dicho color. Por el contrario, si un espacio vacío se encuentra delimitado por piedras de ambos jugadores, se considera que no es \textit{territorio} de ningún jugador, véase Figura~\ref{fig:Fig 9}. Cuando se enfrentan dos personas con distinto nivel de habilidad, se suele otorgar una ventaja al jugador de menor experiencia. Esta ventaja consiste en jugar con las piedras negras (primero) y colocar un número determinado de \textit{piedras} en el tablero antes de comenzar la partida; a mayor diferencia más piedras, véase Figura~\ref{fig:14}.
...
\begin{wrapfigure}[14]{r}{0.35\textwidth}
	...
	\end{tikzpicture}
	\caption{Ventajas en un tablero de $13\times13$.}
	\label{fig:14}
\end{wrapfigure} Esta es la disposición tradicional de las piedras de ventaja (tanto en tableros de $13 \times 13$ como de $19 \times 19$). Existe también la \textit{ventaja libre}, en donde el jugador de menor nivel decide libremente dónde posicionar estas piedras iniciales. 

El objetivo del Go es obtener más \textit{territorio} que el oponente. Durante su turno, un jugador puede optar por pasar y ceder la acción a su rival. La partida finaliza cuando ambos jugadores pasan de manera consecutiva. En ese momento, comienza una fase de acuerdo mutuo en que se identifican y retiran del tablero todas las piedras «muertas» (es decir, aquellas que no se tiene forma de evitar su captura). Tras retirar estas piedras, el ganador se determina sumando los \textit{territorios} asegurados y las piedras vivas que quedan al final de la partida. La Figura~\ref{fig: Fig 10} ilustra una partida terminada. El puntaje se calcularía de la siguiente manera: 11 piedras del jugador negro más 16 espacios en su territorio. Por su parte, el blanco cuenta con 11 piedras y 11 espacios en su territorio. Por lo tanto, el negro obtiene 27 puntos y el blanco 22, de modo que el negro gana por 5 puntos.\footnote{Este método corresponde a la forma china de contar puntos. Hay muchos otras formas, como la japonesa o la coreana.} Es posible cuestionar la victoria del jugador negro en este ejemplo, dado que tuvo la ventaja de realizar el primer movimiento. Para equilibrar esta disparidad, el jugador blanco recibe una compensación por jugar en segundo lugar, denominada \textit{komi}. Esta cantidad varía según el tamaño del tablero y la diferencia en el nivel de habilidad de los jugadores; por lo general, en tableros estándar se utilizan 5,5 puntos.\footnote{El \textit{komi} varía dependiendo del conjunto de reglas elegido. Además, en partidas con piedras de ventaja, se suele aplicar un \textit{komi} mínimo de 0,5 puntos.} Si se aplicara esta regla al ejemplo anterior, el blanco ganaría por 0,5 puntos, el menor margen posible. La inclusión de este medio punto en el \textit{komi} imposibilita los empates.
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[scale=0.485] \tikzstyle{every node}=[font=\small]
		% Draw the background
	...
	\end{tikzpicture}
	\caption{Tablero de una partida terminada, con sus puntos marcados; la piedra negra marcada con el punto \textcolor{Orange}{naranja} debe retirarse por estar muerta.}
	\label{fig: Fig 10}
\end{figure}

Un grupo posee un \textit{ojo} si rodea completamente una intersección vacía. Con al menos dos \textit{ojos}, adquiere el estado de \textit{vivo} y jamás podrá ser capturado. En la Figura~\ref{fig:Fig 12} (izquierda), aunque el jugador blanco intente atacar al grupo negro, solo puede colocar una piedra por turno. Como ocupar esos espacios violaría la regla del suicidio, el blanco necesitaría jugar en ambos ojos simultáneamente para capturar, lo cual es imposible. Por lo tanto, el grupo negro está \textit{vivo}. Existen, sin embargo, los llamados ojos falsos, puntos que aparentan ser \textit{ojos}, pero donde un grupo que lo rodea corre el riesgo de quedar con una sola libertad (la del punto). En consecuencia, si el oponente logra capturar ese grupo vulnerable, la estructura se desarma y el supuesto ojo se pierde, véase Figura~\ref{fig:Fig 12} (derecha).
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.485] \tikzstyle{every node}=[font=\small]
		...
		\end{scope}
	\end{tikzpicture}
	\caption{Ojos de un grupo vivo (\textcolor{Blue}{azul}). Ojo falso (\textcolor{OliveGreen}{verde}). Si blanco juega en el punto \textcolor{Red}{rojo}, negro estará obligado a rellenar su ojo con una piedra o podría perderla.}
	\label{fig:Fig 12}
\end{figure}


Para construir el Go necesitamos de cuatro elementos: \textit{donde}, \textit{quienes}, \textit{qué} y \textit{cómo}.